...
-center mb-3">
                        ...
                        <div class="flex-grow-1 mx-2">
                            <h5 class="mb-0">{{ $noticeSystem->title }}</h5>
                            <span class="text-sm text-muted">{{ $noticeSystem->created_at }}</span>
                        </div>
                    </div>
                    <p class="mb-3">{!! $noticeSystem->content !!}</p>
                    <div class="row my-3">
                        <div class="col">
                    ...
                    <div class="d-flex align-items-center mt-3">
                        <div class="flex-shrink-0">
                            <img class="img-radius d-none d-sm-inline-flex me-3 wid-40 rounded-circle" src="https://ui-avatars.com/api/?background=random&name={{Auth::user()->name ?? 'Bạn chưa đăng nhập'}}" alt="User image">
                        </div>
...
<script>
    ...
    const rechargeData = @json($data['recharge']);
    ...
    (function () {
      var options_order_statistics = {
        ...
        series: [
          ...
          {
            name: 'Đang chờ',
            data: @json($totalPending)
          },
          {
            name: 'Đang chạy',
            data: @json($totalProcessing)
          },
          {
            name: 'Đã huỷ',
            data: @json($totalCanceled)
          },
          {
            name: 'Chờ hoàn tiền',
            data: @json($totalPendingRefundCancel)
          },
          {
            name: 'Hoàn tiền',
            data: @json($totalRefund)
          },
          {
            name: 'Hoàn thành',
            data: @json($totalCompleted)
          }
        ],
        xaxis: {
