        ...
        success: function (response) {
            if (response.code === '200' && response.status === 'success') {
                Swal.fire({
                    ...
                    customClass: {
                        confirmButton: 'btn btn-lamtilo_success btn-success',
                        cancelButton: 'btn btn-lamtilo_cancel btn-secondary' 
                    }
                }).then((result) => {
...
function checkPrice() {
    ...
    elm.append(`
        <div class="alert alert-info bg-info text-white" id="informationServer">
            <h6>${lam}</h6>                               
        </div>
      `);
